...
import { SyllabusLectureProps } from './constants';
import {
  SyllabusCheckIconStyled,
  SyllabusLectureIconStyled,
  SyllabusSubItemStyled,
} from './styled';

export function SyllabusLecture({
  id,
  vertical = false,
  checked = false,
  active = false,
    ...
    >
      <Flex shrink={false}>
        {checked ? (
          ...
          <SyllabusLectureIconStyled vertical={vertical}>
            <Icons.Book />
          </SyllabusLectureIconStyled>
        )}
      </Flex>
      <Flex direction="column">
        <Headline size={4} className="mb-1 mt-0">
          <strong>
            {i18n('ui.components.classroom.lecture', { number })}
            {title ? ':' : ''}
          </strong>{' '}
